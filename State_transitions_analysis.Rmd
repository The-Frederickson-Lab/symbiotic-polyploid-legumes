...
### Pruning for genus level ploidy data 
...
ploidy_genus <- ploidy1 %>% 
  filter(!is.na(Fixer), !is.na(diPloidyLow))

# Make sure the data and tree match up 
TreeOnly <- setdiff(legumes1$tip.label, rownames(ploidy_genus))
DataOnly <- setdiff(rownames(ploidy_genus), legumes1$tip.label)

# Prune the tree 
genus_tree <- drop.tip(legumes1, TreeOnly)
genus_tree$node.label <- NULL

# Get the tip names 
tip_names<-data.frame(genus_tree$tip.label)

# Filter the dataset for tree species 
ploidy_genus2<-ploidy_genus %>%
  rownames_to_column("Species") %>%
  filter(Species %in% tip_names$genus_tree.tip.label) %>%
  dplyr::select(Species, Fixer, diPloidyLow)
...
## Estimating state transitions on genus-level ploidy data 
...
MK_genus_simple<- corHMM(phy=genus_tree, data=ploidy_genus2, rate.cat=1)

# Look at the results of the model 
MK_genus_simple
plotMKmodel(MK_genus_simple)
...
MK_genus_hidden<- corHMM(phy=genus_tree, data=ploidy_genus2, rate.cat=2)

# Look at the results of the model 
MK_genus_hidden
plotMKmodel(MK_genus_hidden, display = "row")
...
Running the same simple model as above on the subfamily/genus combination of ploidy data. 
...
Running the same hidden states model as above on the subfamily/genus combination of ploidy data. 
...
### Genus level ploidy data 

Plotting the transition rates from the hidden state model for the genus level ploidy data.
...
phy_genus_hidden = MK_genus_hidden$phy
data_genus_hidden = MK_genus_hidden$data
model_genus_hidden = MK_genus_hidden$solution
model_genus_hidden[is.na(model_genus_hidden)] <- 0
diag(model_genus_hidden) <- -rowSums(model_genus_hidden)

# Run to get simmap 
simmap_genus_hidden <- makeSimmap(tree = phy_genus_hidden, data = data_genus_hidden, model = model_genus_hidden, rate.cat = 2)

# Import phytools plotSimmap for plotting
phytools::plotSimmap(simmap_genus_hidden[[1]], colors=state_colours8, fsize = 0.05, type="fan")
