...
Look at the species added in the genus+subfamily dataset in terms of fixer status and number of introduced ranges. 
...
We performed all analyses on our two datasets separately: genus level ploidy data and subfamily level ploidy data. We tested tested the hypothesis that ploidy and symbiotic status interact to impact range expansion using pgls models. 
...
summary(model_genus)
anova(model_genus)
# With the logged values the sign of the ploidy estimate is correct, the interaction is non-significant, and fixer sign is opposite 
...
Testing the same hypothesis with a different package to see if the same results are achieved. 
...
summary(model_sub)
anova(model_sub)
# The estimates for ploidy is the proper sign and interaction is non-significant, but the fixer sign is opposite to what is expected 
...
# This might be different than what Megan gets 
...
# Without the phylogeny the estimate for fixer status is different 
...
Our results (which is a subsetted dataset) do look like the non-symbiotic species are introduced to more ranges just like in Simonsen et al 2017. Although a lot of outliers in the symbiotic group... 
